...
Variables: MAX_DAYS(2048);

Vars: ContractValue(0);
Vars: maxEq(0), dwdt(0), dwdTT(0);
...
Vars: nDay(0), winPerc(0), lossPerc(0), NetPerc(0);

Arrays: dwdts[12](0);
Arrays: eqDayClose[3072](0);
...
if Date > lastDate then begin
	...
	nDay = nDay + 1; eqDayClose[nDay] = opEq[1];
	for c1 = 0 to nDay begin
		if eqDayClose[c1] >= opEq[1] then
			if nDay - c1 > dwdTT then begin
				dwdTT = nDay - c1; c1 = nDay; end;
	end;

	if opEq[1] <= maxEq then
		dwdt = dwdt + 1
	else begin
