...
If ( Date <> Date[1] ) then Begin
	Pivot = ( Low data3 + High data3 + Close data3) / 3;

	Resistance1 = ( Pivot * 2 ) - Low data3;
	Resistance2 = Pivot + Range data3;
	Resistance3 = Resistance1 + Range data3;

	Support1 = ( Pivot * 2 ) - High data3;
	Support2 = Pivot - Range data3;
	Support3 = Support1 - Range data3;

	numTDay = 0;							
	HH = H data3; LL = L data3;
End;
...
If ( Time > sess1firstbartime + NotBef) 
	and (Time < NotAft) 
	and (numTDay < MaxTDay)
    and adx(14) data2 < 30 and t <> 1430 then 
	Begin
		{ Placement of Short orders }
		If Low data3 > Support3 then 
		Begin
		...
		If High data3 < Resistance3 then 
		Begin
			{DefineDLLFunc:"RACRT.dll",int,"CONTROLLER","AX ","EM Harvard","2"int,int,int,float;}
...
{Reverse}
if (MarketPosition > 0) 
	and adx(14) data3 < 25 and t <> 1430 
...
if (MarketPosition < 0) 
	and adx(14) data2 < 25 and t <> 1430 
	and OBV data2 >= average(OBV data2, 20) 
	then begin
		Reverse = (EntryPrice * RevPerc) / 100;	 
		if (numTDay < MaxTDay) then {reverse}
...
if HH < H data3 then HH = H data3;
if LL > L data3 then LL = L data3;
...
if marketposition = 1 then sell ("maxl") next bar at entryprice*0.99 stop;
if marketposition = -1 then buytocover ("maxs") next bar at entryprice*1.01 stop;

if currentcontracts <= nos/2 THEN BEGIN
   if marketposition = 1 then sell ("DayL") next bar at lowd(0) -1 stop;
   if marketposition = -1 then buytocover ("DayS") next bar at highd(0) +1 stop;
END;
