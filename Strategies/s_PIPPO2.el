...
Input: EMALen(162),KLen(3),DLen(7),OverSold(20),OverBougth(80);
Input: MoneyManagement (1);
...
if StartTime < value1 and value1 < EndTime then begin 
 ...
 if marketposition = 0 then begin
  ...
  if canSell and condition2 then begin 
   ...
   else if c <= sellLevel then begin 
    sell short("es.m") this bar c;
   end;
 ...
 if MoneyManagement = 1 and barssinceentry > 0 then begin
 ...
 if marketposition = 1 then begin  
  ...
  if not mkt then begin
   
   if entryprice + mcp < secondTarget then 
    stpv = entryprice + mcp - (cref - risk)
   else stpv = entryprice + mcp - IntPortion(0.5*(cref - risk) + 0.5);
   
   stp = c >  stpv;
   mkt = c <= stpv;
   
   if stp then sell("XL.TrStop") next bar stpv stop
   else if mkt then sell("XL.TrStop.M") this bar c;
  ...
  if not mkt then begin
   ...
   if entryprice - mcp > secondTarget then 
    stpv = entryprice - mcp + (risk - cref)
   else stpv = entryprice - mcp + IntPortion(0.5*(risk - cref) + 0.5);
   
   stp = c <  stpv;
   mkt = c >= stpv;
   
   if stp then buy to cover("XS.TrStop") next bar stpv stop
   else if mkt then buy to cover("XS.TrStop.M") this bar c;
