...
if t > 800 and t < 2200 then begin
 ...
 if marketposition <> 0 then begin
  ...
  if marketposition = 1 then begin
   ...
   {
   //TRAILING STOPLOSS
   stopval = entryprice + (mcp - (stopl - currentcontracts*(slippage+commission))/bigpointvalue);
   stp     = mcp < brkl/bigpointvalue and minstop < stopval and stopval < c;
   mkt     = mcp < brkl/bigpointvalue and c < stopval;
   
   if stp then sell("xl.stpbr") next bar at stopval stop;
   if mkt then sell("xl.stpbr.m") next bar at market;
   }
   //PROFIT TARGET
   ...
   {
   //TRAILING STOPLOSS
   stopval = entryprice - (mcp - (stops - currentcontracts*(slippage+commission))/bigpointvalue);
   stp     = mcp < brks/bigpointvalue and c < stopval and stopval < maxstop;;
   mkt     = mcp < brks/bigpointvalue and c > stopval;
   
   if stp then buytocover("xs.stpbr") next bar at stopval stop;
   if mkt then buytocover("xs.stpbr.m") next bar at market;
   }
   //PROFIT TARGET
...
condition1 = d >= 1090121;

if {condition1 and} adxv < adxlimit and minstop < lok and upk < maxstop and engine then begin
