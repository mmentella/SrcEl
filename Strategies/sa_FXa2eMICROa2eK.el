...
if currentbar > 1 then begin
 ...
 input: lenl(8),kl(2.1),lens(32),ks(4);
 
 input: stopl(33),funkl(24.0),brkl(25),tl(35);
 input: stops(29),funks(15.5),brks(17),ts(56);
 
 var: upb(0),lob(0),el(true),es(true),engine(true),orderL(true),orderS(true);
 var: trade(0),fnkl(false),fnks(false),dru(0),mcp(0),goL(true),goS(true);
 ...
 if trade < 1 and engine then begin
  ...
  orderL = not fnkl and goL and marketposition < 1 and el;
  if orderL then 
   buy("el") next bar upb stop;
  
  orderS = not fnks and goS and marketposition > -1 and es;
  if orderS then
