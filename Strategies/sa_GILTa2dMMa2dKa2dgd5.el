...
Inputs: _StopLossLong(1550), _StopLossShort(1900);
Inputs: _DayContractStopLong(1150), _DayContractStopShort(1700);
...
Inputs: _ModExitShort(1450), _TargetShort(3400);
Variables: _BrkStartShort(0);
Variables: _TrailStartShort(0), _TrailPercShort(0), _TrailDeltaShort(0), _TrailEndPercShort(0);
 ...
 ***********************}
	...
	dayTrade = gd.MoneyMngV1A(StopOrderPrice, StopOrderLabel,	ModalOrderPrice, TargetOrderPrice,
	                          NoS, {MarketPosition * EntryPrice}MarketEntry,
	                          {LossMng}true,
	                          _DailyMaxLoss, {settlePrice,} _StopLossLong, _StopLossShort, {_StopLossAlgebric}false,
	                          MoneyMng,
	                          _ModExitLong, _TargetLong, _BrkStartLong,
	                          _TrailStartLong, _TrailPercLong, _TrailDeltaLong, _TrailEndPercLong,
	                          _ModExitShort, _TargetShort, _BrkStartShort,
	                          _TrailStartShort, _TrailPercShort, _TrailDeltaShort, _TrailEndPercShort
	                         );
	...
	if marketposition < 0 then begin
		stopVal = yc + _DayContractStopShort/bigpointvalue;
		if stopVal < StopOrderPrice then begin
			StopOrderPrice = stopVal; StopOrderLabel = lblDayStop; end;
	end;
 ...
 *****************}
	...
	if MarketEntry < 0 then begin { of LONG Position Management }

		{ SHORT - Put nearest STOP order at Market }
		if StopOrderPrice < EntryLongPrice - tradeCost then
			if Close > StopOrderPrice - tickUnit then begin
				switch StopOrderLabel begin
					case lblStopLoss:     BuyToCover ("XS-Stop.m") NumContracts shares next bar at Market;
					case lblStopAlgebric: BuyToCover ("XS-StAl.m") NumContracts shares next bar at Market;
					...
					case lblDayStop:      BuyToCover ("XS-DayStop.m") NumContracts shares next bar at Market;
					case lblNone:         ;
					default:              Abort;
				end;
			end else
			if StopOrderPrice < stopMax then begin
				switch StopOrderLabel begin
					case lblStopLoss:     BuyToCover ("XS-Stop.s") NumContracts shares next bar at StopOrderPrice Stop;
					case lblStopAlgebric: BuyToCover ("XS-StAl.s") NumContracts shares next bar at StopOrderPrice Stop;
					case lblHLbars:       BuyToCover ("XS-HBar.s") NumContracts shares next bar at StopOrderPrice Stop;
					case lblATR:          BuyToCover ("XS-ATR.s") NumContracts shares next bar at StopOrderPrice Stop;
					case lblTrailing:     BuyToCover ("XS-Trl.s") NumContracts shares next bar at StopOrderPrice Stop;
					case lblBreakEven:    BuyToCover ("XS-Brk.s") NumContracts shares next bar at StopOrderPrice Stop;
					case lblDayStop:      BuyToCover ("XS-DayStop.s") NumContracts shares next bar at StopOrderPrice Stop;
					case lblNone:         ;
					default:              Abort;
				end;
			end;

		if ModalOrderPrice > 0 then
			if ModalOrderPrice >= Low - tickUnit then
				BuyToCover ("XS-Modal.m") NumModal shares next bar at Market
			else
			if ModalOrderPrice > stopMin then
				BuyToCover ("XS-Modal.s") NumModal shares next bar at ModalOrderPrice Limit;
		
		if TargetOrderPrice > 0 and (ModalOrderPrice <= 0 or TargetOrderPrice < ModalOrderPrice) then
			if TargetOrderPrice >= Low - tickUnit then
				BuyToCover ("XS-Target.m") NumModal shares next bar at Market
			else
			if TargetOrderPrice > stopMin then begin
				numCntr = IFF(ModalOrderPrice > 0, NumSpare, NumContracts);
				BuyToCover ("XS-Target.s") numCntr shares next bar at TargetOrderPrice Limit;
			end;
