Inputs: Nos(2), MaxTrade(1),rocs(0),rocl(0);

{ Strategy Inputs }
Inputs: MinLenL(15), MaxLenL(23), MinLenS(2), MaxLenS(25), lungSTDL(30),lungSTDS(30);
Inputs: lungadx(18), bandaadx(30);

{ Process Inputs }
Inputs: TimeIN(1100), TimeOUT(1700); 
...
Input: SStopL(0), SStopS(0), BRKL(0), BRKS(0), TargetL(0), TargetS(0), TargetL2(0), TargetS2(0),
       ...
       StopDayL(0), StopDayS(0), StopDayLNP(0), StopDaySNP(0), 
       TargetLDay(0), TargetSDay(0), TargetLDayNP(0), TargetSDayNP(0);

Input: StopLimitPerc(0), SettleTime(0); 


{ Strategy Variables }
Vars: X(0, data2), Y(0, data2), XS(0, data2), YS(0, data2), Variazione(0, data2), VA(0, data2), VB(0, data2), OldA(0, data2), OldB(0, data2), CanADX(false, data2),longentry(0.data2),shortentry(0.data2);
...
if BarStatus(2) = 2 then begin
	...
	If CurrentBar of data2 = 1 then begin 
		VA = 20; 
		VB = 20;
		X = StddevPino(Close of data2 , lungSTDL) of data2;
		XS = StddevPino(Close of data2 , lungSTDS) of data2; 
	end; 

	Y = X[rocl];
	X = StddevPino(Close of data2, lungSTDL) of data2; 
	if X <> 0 then Variazione = (X - Y) / X;

	OldA = VA;
	VA = OldA * (1 + Variazione);
	VA = MaxList(VA, MinLenL);
	VA = MinList(VA, MaxLenL);
	
	YS = XS[rocs];
	XS = StddevPino(Close of data2, lungSTDS) of data2; 
	if XS <> 0 then Variazione = (XS - YS) / XS;
	
	OldB = VB;
	VB = OldB * (1 + Variazione);
	VB = MaxList(VB, MinLenS);
	VB = MinList(VB, MaxLenS); 
	
	longentry = Highest(h,IntPortion(va))data2;
	shortentry = Lowest(l,IntPortion(vb))data2;
...
// Market StartUp
if time > TimeIN and time < TimeOUT then begin
 ...
 begin
  if TradesToday(date) < MaxTrade {and enableTrade} then 
   begin
   Buy("EL.S") Nos share next bar at longentry stop;
   //enableTrade = false;
   end;
end;

 if marketposition <> -1 and CanADX then begin
  if TradesToday(date) < MaxTrade {and enableTrade} then begin
   Sell Short("ES.S") Nos shares next bar at shortentry stop;
   //enableTrade = false;
  end;
 end;
 //end;
