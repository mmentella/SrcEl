...
vars: uppk(0,data2),lowk(0,data2),adxval(0,data2),mcp(0),bounce(false);

uppk = KeltnerChannel(h,len,kl)data2;
lowk = KeltnerChannel(l,len,-ks)data2;
...
//ENGINE
if absvalue(len-MM.DominantCyclePeriod(medianprice)data2) < diffper and TradesToday(d) = 0 then begin
 if marketposition < 1 and c < uppk and c data2 < uppk then
  buy("el.s") nos shares next bar at uppk stop;
 if marketposition > -1 and c > lowk and c data2 > lowk then
