...
if time > 800 and time < 2230 then begin
 ...
 if marketposition = 1 then begin  
  ...
  if step > 0 then begin
   ...
   if fl = false then begin
    if (entryprice + level) > close then
     sell("flx@m") entry("fl") this bar on close 
    else sell("flx") entry("fl") next bar at entryprice + level stop;
   end;
   if rl = false then begin 
    if (entryprice + level) > close then
     sell("rlx@m") entry("rl") this bar on close
    else sell("rlx") entry("rl") next bar at entryprice + level stop;
   end;
  end;
 end;//mp = 1
 ...
 if marketposition = -1 then begin 
  ...
  if step > 0 then begin
   ...
   if fs = false then begin
    if (entryprice + level) < close then
     buytocover("fsx@m") entry("fs") this bar on close
    else buytocover("fsx") entry("fs") next bar at entryprice + level stop;
   end;
   if rs = false then begin
    if (entryprice + level) < close then
     buytocover("rsx@m") entry("rs") this bar on close
    else buytocover("rsx") entry("rs") next bar at entryprice + level stop;
   end;
  end;
 end;//mp = -1
