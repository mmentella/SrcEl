Inputs: nos(2),lenl(22),kl(2.61),lens(23),ks(2.05),alpha(.21),adxlen(19),adxlimit(26),sod(800),eod(1930);
Inputs: stopl(1500),brkl(1700),modl(1600),tl(7600);
...
if t > sod and t < eod then begin
 ...
 if adxv < adxlimit then begin
  ...
  if marketposition > -1 and ssetup then begin
   stopLbl = lblEnter;
   if c <= lok then begin
    sell("ES.m") nos shares next bar at market; stopOne = hugePrice; end
   else
   if StopMin < lok and lok < StopMax then begin
    sell("ES.s") nos shares next bar at lok stop; stopOne = lok; end;
  end;
 end;
